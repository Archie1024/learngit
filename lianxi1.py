...
# print(type(dict1))#花括号，不一定是字典，因为字典一定要是键值串出现的，一个键代表一个元素。
...
# 所以定义字典的时候尽量用字符串，否则展开的时候回出错的。而且用int当key和下标差不多，也看不懂！！
#字典的键也尽量不要和变量重名
...
# import BBBB.sum2 as sm
# sm.get_sum(1,100)

"""""""""""""""""""""""""也可以只导入一个函数,这种写法有个弊端，一般只能一次导入一个"""""""""""""""""""""""""""""""""""""
# from AAAA.sumfun import get_sum2
# get_sum2(1,2)   #这时候就是仅导入一个函数了，但是为什么仅导入一个函数也会运行函数之前的代码呢?因为导入即执行
...
3、from phone.mobile.analog import dial
dial()        仅导入某个函数，可以直接调用
...
"""
#从某包某模块内倒入某函数！！
from AAAA.sumfun import  get_sum2    #demo3里面的if __name__ == "__main__":下的测试代码就不糊运行了！
get_sum2(start = 0,end = 100)
"""疑问：但是这里为什么仅引用函数，也会运行函数前后整个模块的内容"""#疑问疑问！！！！！！！！！！
...
"""
# 代码编辑
# structur  能看到整个模块里面所有的函数和变量
#在里面新增数据库，settings--plugins,里面可以安装各种插件，比如数据库管理插件，比如SVN插件
#版本管控能，用svn控制发布代码，可以用版本管控工具
# project lesson 可以换解释器，或者直接安装和卸载第三方库
#怎么看查看函数在哪里被调用，先ctrl加鼠标左键回道定义的地方，再Ctrl悬停鼠标，能够查看在哪里被调用。
#怎么查看函数在哪里被调用，直接选中函数，鼠标左键，find usages,能够找到函数在哪里被调用。
#一切报错以控制台输出为准，因为有时候当前对对话框没有感知到已经导入的模块。
#查看历史代码，如果代码不小心Ctrl + C已经无法补救了，那么可以用local history,本地历史功能补救，在模块右键补救。
#能够找到之前的历史版本
#批量重构，
# 1、一个一个的改
# 2、找到任何一个变量或函数的名字，右键refactor—rename，直接改掉所有相关的名字。变量名或者函数名，或者类名
"""增加根目录，模块搜索路径设置"""
...
#         # print("我被执行了")
#         self.weight = inWeight#再赋值，重新再赋值。实例属性是针对个人的
#
